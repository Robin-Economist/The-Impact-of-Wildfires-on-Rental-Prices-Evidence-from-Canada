...
**Table layout**: The regional-absorption table (province-by-year and region-by-year fixed effects) was reset from landscape to portrait — five columns fit upright, removing a near-empty rotated page and shortening the document by two pages. Added a `\FloatBarrier` before the Robustness section so the placebo-diagnostics figure no longer floats above its section heading. Harmonized the notes of the COVID-truncation / distant-placebo table into the `threeparttable` block, matching the other regression tables. Dropped the `\resizebox` on the three-column migration-flow diagnostics table, which had been scaled to full text width and inflated to fill an entire page; at its natural `\small` size it now sits inline with the text. Applied the same fix to the four-column supply-side heterogeneity table in the appendix. A pass over the remaining `\resizebox` tables confirmed the scaling is needed there — the five-column specification tables carry wide multi-word column headers and overrun the text width without it. Cleared the last remaining overfull line (a gravity-validation paragraph in the appendix) with a local `\emergencystretch`; the document now compiles with no overfull boxes.
